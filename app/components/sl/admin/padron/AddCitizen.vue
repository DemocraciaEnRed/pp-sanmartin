  ...
  <i class="fas fa-exclamation-triangle fa-fw"></i> No agregue al padron ciudadanos que no hayan sido verificados.
    ...
    <div class="columns">
      ...
      <div class="column">
        <div class="field">
           <div class="control">
          <label class="label is-size-4" :class="{'has-text-danger': errors.has('district')}">
            <i class="fas fa-angle-double-right"></i> Barrio *</label>
          <div class="control">
              <div class="select is-large is-fullwidth">
                <select name="district" v-model="citizen.district_id" v-validate="'required'">
                  <option :value="null">Seleccione el barrio</option>
                  <option
    ...
    <div v-else>
      ...
      <div class="columns">
        <div class="column" v-if="allowRestart">
          <button @click="restart()" class="button is-primary is-outlined is-fullwidth">
            <i class="fas fa-undo fa-fw"></i>&nbsp;Reiniciar
          </button>
        </div>
        <div class="column" v-if="allowStartVoting">
        <button @click="startVoting" class="button is-link is-fullwidth">
...
<script>
export default {
  props: ["url", "allowRestart","allowStartVoting", "urlParticipacion", 'districts'],
  data() {
    return {
      isLoading: false,
      response: {
  ...
  computed: {
    payload: function() {
      return {
        ...
        district_id: this.citizen.district_id
      };
    },
