<template>
  <section>
    <form-project ref="formProject" :project.sync="project" :budget="budget" :user="user" :citizen="citizen" :editable="editable" :district="district"></form-project>
    <hr>
    <h1 class="title is-4 has-text-centered">
          ¿Todo listo?
      </h1>
      <h1 class="subtitle is-6 has-text-centered">
          Cuando hayas terminado, apretá en <b><i class="fas fa-save"></i> Guardar</b>.<br>¡Y no te preocupes! Vas a poder editarlo las veces que quieras, hasta que cierre la fecha para la subida de proyectos
      </h1>
    ...
    <div class="message is-success" v-show="response.replied && response.ok">
      <div class="message-body">
        <i class="fas fa-check fa-fw"></i>&nbsp;<b>¡Genial!</b> ¡Tu proyecto ha sido guardada correctamente! ¡Felicidades y gracias por tu contribución!
      </div>
    </div>
...
export default {
  props: ["formUrl", "budget", "user", "citizen", "editable", "district"],
  components: {
    FormProject
  },
  data() {
        ...
        .then(response => {
          if (response) {
            ...
            this.$http
              ...
              .then(response => {
                this.$snackbar.open({
                  message: "El proyecto ha sido guardado guardado con éxito!",
                  type: "is-success",
                  actionText: "¡Genial!"
                });
  ...
  computed: {
    payload: function() {
      ...
      if (this.project.withOrganization) {
