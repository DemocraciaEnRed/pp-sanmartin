...
class UserPanelAction
{
    ...
    public function showCreateProject($request, $response, $params)
    {
        ...
        $projectsCount = $this->db->query('App:Project', ['district'])
            ->where('author_id', $user->id)->count();
        if($projectsCount >= 1) {
            throw new AppException('Ya ha subido un proyecto. El limite para subir proyectos es hasta un (1) por ciudadano de San Martín.');
        }
